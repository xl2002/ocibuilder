...
```bash
# 登录到远端
./output/main.exe login --username admin --password Harbor12345 192.168.253.128:8080
# linux系统测试样例
...
```bash
# 退出登录
./output/main.exe logout 192.168.253.128:8080
...
```bash
# 从远端拉取镜像
./output/main.exe pull 192.168.253.128:8080/library/busybox-image:latest
...
```bash
# 推送镜像到远端
./output/main.exe push 192.168.253.128:8080/library/busybox-image:latest
# 推送镜像到本地目录
