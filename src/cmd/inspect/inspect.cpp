...
void inspectCmd(Command& cmd, vector<string> args){
    ...
    builderInfo.FromImageID = imagestore->ID;
    ...
    if (imagestore->image_manifest) {
        boost::json::value manifestJson = boost::json::value_from(*imagestore->image_manifest);
        builderInfo.Manifest = boost::json::serialize(manifestJson);
    } else {
        builderInfo.Manifest = "";
    }
    ...
    builderInfo.ImageAnnotations = imagestore->Flags;
    builderInfo.ImageCreatedBy = "";  // 假设创建者为空
