...
void Builder::Save(std::string name){
    ...
    imagestore->image_index->historyTags.push_back(olddigest);
    // imagestore->image_index->annotations=manifest->Annotations;
    imagestore->image_index->digest=manifestDigest->String();
    // imagestore->image_index->mediaType=manifest->MediaType;
    imagestore->image_index->size=manifestBytes.size();

    auto images=std::dynamic_pointer_cast<ImageStore>(this->store->image_store);
    images->Save();
