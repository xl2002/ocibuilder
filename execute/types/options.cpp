...
bool loadDefaultStoreOptions() {
    ...
    setDefaults();
    const char* env_p = boost::compute::detail::getenv("CONTAINERS_STORAGE_CONF");
    string path = env_p ? env_p : "";
    // std::string path = string(getenv(string("CONTAINERS_STORAGE_CONF").c_str()));
    if (!path.empty()) {
    ...
    const char* xdgConfigHome_p = boost::compute::detail::getenv("XDG_CONFIG_HOME");
    string xdgConfigHome = xdgConfigHome_p ? xdgConfigHome_p : "";
    // std::string xdgConfigHome =string (getenv(string("XDG_CONFIG_HOME").c_str()));
...
std::string DefaultConfigFile() {
    ...
    ///<通过环境变量获取路径
    const char* path = boost::compute::detail::getenv(storageConfEnv.c_str());
    if (path) {
        return path;
    }
