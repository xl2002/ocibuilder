...
std::shared_ptr<Entry> Logger::newEntry() {
    std::lock_guard<MutexWrap> Lock(mu);
    if (!entryPool.empty()) {
...
void Logger::Log(Level level, const std::vector<std::any>& args) {
    if (IsLevelEnabled(level)) {
        auto entry = newEntry();
...
void Logger::Trace(const std::vector<std::any>& args) {
    Log(level::TraceLevel, args);
}

void Logger::Debug(const std::vector<std::any>& args) {
    Log(level::DebugLevel, args);
}

void Logger::Info(const std::vector<std::any>& args) {
    Log(level::InfoLevel, args);
}

void Logger::Print(const std::vector<std::any>& args) {
    auto entry = newEntry();
    entry->Print(args);
    releaseEntry(entry);
}
...
void Logger::Warning(const std::vector<std::any>& args) {
...
void Logger::Fatal(const std::vector<std::any>& args) {
    Log(level::FatalLevel, args);
    Exit(1);
}

void Logger::Panic(const std::vector<std::any>& args) {
...
void Logger::Logln(const Level &level, const std::vector<std::any> &args)
{
    if (IsLevelEnabled(level))
    {
        auto entry = newEntry();
...
void Logger::Println(const std::vector<std::any> &args)
{
    auto entry = newEntry();
    entry->Println(args);
    releaseEntry(entry);
...
void Logger::Traceln(const std::vector<std::any> &args)
{
    Logln(level::TraceLevel, args);
}

void Logger::Debugln(const std::vector<std::any> &args)
{
    Logln(level::DebugLevel, args);
}

void Logger::Infoln(const std::vector<std::any> &args)
{
    Logln(level::InfoLevel, args);
}

void Logger::Warnln(const std::vector<std::any> &args)
{
    Logln(level::WarnLevel, args);
}

void Logger::Warningln(const std::vector<std::any> &args)
{
    Warnln(args);
}

void Logger::Errorln(const std::vector<std::any> &args)
{
    Logln(level::ErrorLevel, args);
}

void Logger::Fatalln(const std::vector<std::any> &args)
{
    Logln(level::FatalLevel, args);
    Exit(1);
}

void Logger::Panicln(const std::vector<std::any> &args)
{
...
void Logger::AddHook(const Hook& hook) {
    mu.Lock();
    Hooks.Add(std::make_shared<Hook>(hook));
    mu.Unlock();
}
...
LevelHooks Logger::ReplaceHooks(const LevelHooks& newHooks){
    mu.Lock();
    auto oldHooks = Hooks;
    Hooks = newHooks;
    mu.Unlock();
