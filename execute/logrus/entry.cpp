...
void Entry::Trace(const std::vector<std::any>& args) {
    Log(level::TraceLevel, args);
}
void Entry::Debug(const std::vector<std::any>& args) {
    Log(level::DebugLevel, args);
}
// Print 方法调用 Info 方法
void Entry::Print(const std::vector<std::any>& args) {
    Info(args);
}
// Info 方法调用 Log 方法
void Entry::Info(const std::vector<std::any>& args) {
    Log(level::InfoLevel, args);
}

void Entry::Warn(const std::vector<std::any>& args) {
    Log(level::WarnLevel, args);
}

void Entry::Warning(const std::vector<std::any>& args) {
    Warn(args);
}

void Entry::Error(const std::vector<std::any>& args) {
    Log(level::ErrorLevel, args);
}

void Entry::Fatal(const std::vector<std::any>& args) {
    Log(level::FatalLevel, args);
    LoggerPtr->Exit(1);
}

void Entry::Panic(const std::vector<std::any>& args) {
...
void Entry::Logf(const Level& level, const std::string& format, ...) {
    if (LoggerPtr && LoggerPtr->IsLevelEnabled(level)) {
        ...
        // 调用 Log 方法
        Log(level, std::vector<std::any>{std::string(buffer)});
    }
}

// 主日志方法，接受任意参数并处理
void Entry::Log(const Level& level, const std::vector<std::any>& args) {
...
void Entry::Logln(const Level& level, const std::vector<std::any>& args) {
    if (LoggerPtr->IsLevelEnabled(level)) {
...
void Entry::Traceln(const std::vector<std::any> &args)
{
    Logln(level::TraceLevel, args);
}
void Entry::Debugln(const std::vector<std::any> &args)
{
    Logln(level::DebugLevel, args);
}
void Entry::Infoln(const std::vector<std::any> &args)
{
    Logln(level::InfoLevel, args);
}
void Entry::Println(const std::vector<std::any> &args)
{
    Infoln(args);
}
void Entry::Warnln(const std::vector<std::any> &args)
{
    Logln(level::WarnLevel, args);
}
void Entry::Warningln(const std::vector<std::any> &args)
{
    Warnln(args);
}
void Entry::Errorln(const std::vector<std::any> &args)
{
    Logln(level::ErrorLevel, args);
}
void Entry::Fatalln(const std::vector<std::any> &args)
{
    Logln(level::FatalLevel, args);
    LoggerPtr->Exit(1);
}
void Entry::Panicln(const std::vector<std::any> &args)
{
...
std::shared_ptr<Entry> Entry::Dup() const {
...
void Entry::fireHooks() {
    ...
    {
        std::lock_guard<MutexWrap> guard(LoggerPtr->mu);
        tmpHooks = LoggerPtr->Hooks;
    }

    // 触发对应级别的所有钩子
    try {
        tmpHooks.Fire(*LevelPtr, shared_from_this());
    } catch (const std::exception& e) {
        throw myerror("Hook execution error: "+std::string(e.what()));
    }
}
