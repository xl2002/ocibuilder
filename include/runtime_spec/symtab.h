...
struct moduledata {
    pcHeader* pcHeaderPtr;                        // 指向 pcHeader 的指针
    ...
    std::map<int32_t, _type*> typemap;            

    bool bad;                                     

    moduledata* next;                             // 指向下一个 moduledata
};

// funcInfo 结构体
struct funcInfo {
    _func* funcPtr;        // 指向 _func 的指针
    moduledata* datap;     // 指向 moduledata 的指针
};
...
struct Frame {
    ...
    Func* funcPtr;          // 函数指针，用于表示当前帧所对应的函数   
    ...
    Frame(uintptr_t pc = 0, Func* func = nullptr, std::string function = "",
