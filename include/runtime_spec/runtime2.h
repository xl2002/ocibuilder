...
struct _func {
    ...
    std::shared_ptr<funcFlag> flag=nullptr;       // 函数标志
...
struct itab {
    std::shared_ptr<interfacetype> inter=nullptr; // 指向 interfacetype 的指针
    std::shared_ptr<_type> _type=nullptr;         // 指向 _type 的指针
