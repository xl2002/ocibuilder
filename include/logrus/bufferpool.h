...
private:
    std::queue<std::unique_ptr<Buffer>> pool; // 存储 Buffer 的队列
...
public:
    ...
    // 回收 Buffer 到池中
    void Put(std::unique_ptr<Buffer> buffer) override {
    ...
    std::unique_ptr<Buffer> Get() override {
        std::unique_lock<std::mutex> lock(mtx);
        if (pool.empty()) {
            return std::make_unique<Buffer>(); // 创建新的 Buffer
        }
