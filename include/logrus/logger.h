...
// 定义 LogFunction 类型  一个返回std::vecter<std::any>的函数
using LogFunction = std::function<std::vector<std::any>()>;
...
public:
    ...
    std::shared_ptr<Formatter> FormatterPtr;
    ...
    std::shared_ptr<BufferPool> BufferPoolPtr;
    ...
    Logger()
        : Out(std::make_shared<std::ofstream>()), // 默认输出到 stderr
          ...
          Hooks(), // 默认初始化为空的 LevelHooks
          currentLevel(level::InfoLevel), // 默认日志级别为 Info
          ExitFunc([]() { std::exit(EXIT_FAILURE); }), // 默认退出函数
    ...
    void Log(Level level, const std::vector<std::any>& args);
    ...
    LevelHooks ReplaceHooks(const LevelHooks& newHooks);
    void SetBufferPool(const std::shared_ptr<BufferPool>& pool);
};
