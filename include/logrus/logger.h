...
public:
    ...
    // 或在致命错误时转储核心。
    std::shared_ptr<LevelHooks> Hooks=nullptr;
    ...
    std::shared_ptr<Formatter> FormatterPtr=nullptr;
    ...
    // 用于同步写入日志。默认启用锁定
    std::shared_ptr<MutexWrap> mu=nullptr;
    // 可重复使用的空条目
    std::shared_ptr<Pool> entryPool=nullptr;
